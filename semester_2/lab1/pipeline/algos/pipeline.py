...
class Flow(FlowSpec):
    ...
    def start(self):
        ...
        self.next(self.fastqc, self.bwa)

    # Можем выполнять параллельно т.к. не зависит от других step
    @step
    def fastqc(self):
    ...
    @step
    def bwa(self):
        subprocess.run(['bwa', 'index', '-p', f'./output/{self.ref_file}', self.ref_file])
        subprocess.run(['bwa', 'mem', f'./output/{self.ref_file}', self.seq_file], stdout=open('./output/alignments.sam', 'w+'))
